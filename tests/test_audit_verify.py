...
def isolated(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gw, "activity_log_path", lambda: log)
    monkeypatch.setattr(gw_cmd, "activity_log_path", lambda: log)
    monkeypatch.setattr(gw, "load_gateway_registry", lambda: {"gateway": {"gateway_id": "gw-test"}})
    return log
...
def test_first_record_has_seq_1_and_null_prev_hash(isolated):
    rec = gw.record_gateway_activity("gateway_login", username="alice")
    assert rec["seq"] == 1
    assert rec["prev_hash"] is None


def test_subsequent_record_increments_seq_and_hashes_prior_line(isolated):
    gw.record_gateway_activity("gateway_login", username="alice")
    first_line = isolated.read_text().splitlines()[0]
    rec = gw.record_gateway_activity("managed_agent_added", agent_name="echo-demo")
    assert rec["seq"] == 2
    assert rec["prev_hash"] == _hash(first_line)


def test_chain_written_to_disk_matches_in_memory_record(isolated):
    gw.record_gateway_activity("gateway_login")
    gw.record_gateway_activity("managed_agent_added", agent_name="x")
...
def test_verify_passes_on_clean_chain(isolated):
    gw.record_gateway_activity("gateway_login")
    gw.record_gateway_activity("managed_agent_added", agent_name="x")
    gw.record_gateway_activity("asset_bound", agent_name="x")
...
def test_failure_messages_carry_no_raw_record_fields(isolated):
    gw.record_gateway_activity("gateway_login", token="axp_u_SECRET", username="alice")
    gw.record_gateway_activity("managed_agent_added", agent_name="x")
    lines = isolated.read_text().splitlines()
    tampered = json.loads(lines[0])
...
def test_legacy_records_skipped_in_default_mode(isolated):
    ...
    gw.record_gateway_activity("gateway_login")  # seq=1, fresh chain
    report = verify_chain(isolated)
    assert report.ok is True
...
def test_from_seq_skips_earlier_records(isolated):
    gw.record_gateway_activity("a")
    gw.record_gateway_activity("b")
    gw.record_gateway_activity("c")
    lines = isolated.read_text().splitlines()
    tampered = json.loads(lines[0])
    tampered["event"] = "tampered"
    lines[0] = json.dumps(tampered, sort_keys=True)
