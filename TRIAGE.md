...
on one-week sprints that run **Wednesday to Tuesday**. Labels are the swimlanes,
the pull order is top-down, and self-assignment is how we avoid duplicate
effort.

> **GitHub is the engineering board of record; Jira mirrors it at the planning
> layer.** Leadership keeps Jira in sync during the weekly grooming pass — it's
> where epics and initiatives roll up for the broader program view. You work out
> of GitHub issues; you don't have to touch Jira. "No off-board work" below
> means no engineering work tracked *only* somewhere other than a GitHub issue —
> not that Jira is gone.
...
#### `epic: <N.M>-<short-name>` — grouping roadmap work

When a body of roadmap work is big enough to need grouping, leadership creates
an epic label and applies it to the member issues during grooming. Epic labels
follow the initiative numbering: **`epic: <N.M>-<short-name>`**, e.g.
`epic: 1.6-onboarding` or `epic: 3.3-llm-hot-swap`, where `<N.M>` ties the epic
to its initiative (`N`) and epic (`M`) in the roadmap. (See `gh label list` for
the epics currently open.)
...
- **At least one `roadmap` issue per person, per sprint** (the Wednesday-to-
Tuesday week). Beyond
...
The pipeline labels and `ux` have already been created in this repository; the
block below is the canonical definition (name, color, description) so they can
be recreated or audited if a fork or sibling repo needs the same setup:
...
Epic labels are created on demand by leadership as epics are opened, named
`epic: <N.M>-<short-name>` after their initiative/epic number and all with the
same color so they read as one family:

```bash
gh label create "epic: 1.6-onboarding" --color 5319e7 --description "<one-line epic summary>"
```
